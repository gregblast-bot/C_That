...
int main() {
    ...
    for (T = 0; T <= 200; T++) {
        ...
        if(cpuUtil > 1){
			printf("Utilization is greater than 1! Can not run processes.");
		}
        ...
        if (T % periodA == 0 && T % periodB != 0) {
            ...
            if (tB < execTimeB) { 
                ...
                if(finishTimeA > absDeadlineA){
					printf("process A%d missed the deadline, not schedulable!\n", jA);
				}
                // End of your code
        ...
        if (T % periodA != 0 && T % periodB == 0) {
            ...
            absDeadlineB = T + periodB;
            int finishTimeB = execTimeB + T;
            tB = 0;
            if (tA < execTimeA) { 
				// process A is unfinished yet
                // Your code here to handle preemption
                // Please print out the decision
                printf("when T=%d, program switched to run process B%d!\n", T, jB);
                doB = 1;
                ...
                if(finishTimeB > absDeadlineB){
					printf("process B%d missed the deadline, not schedulable!\n", jB);
				}
                // End of your code
